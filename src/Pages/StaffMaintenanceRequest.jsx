...
const handleResolveRequest = async (requestId) => {
  try {
    const response = await api.patch(`/maintenance-request/resolve/${requestId}`);
    if (response.status === 200) {
      // Update the request status locally
      setRequests((prevRequests) =>
        prevRequests.map((request) =>
          request._id === requestId ? { ...request, status: 'Resolved' } : request
        )
      );
  ...
  const getStatusIcon = (status) => {
    ...
    switch (status) {
      case 'Resolved': return <CheckCircle {...iconProps} color="#16a34a" />;
      case 'In Progress': return <RefreshCw {...iconProps} color="#eab308" />;
      default: return <Wrench {...iconProps} color="#ea580c" />;
    }
  };
